...
satoo様のホームページより。
###基本
データ型は存在せず、全て文字列です。文字列の内容が数字である時に限って数として取り扱えます。数字は32ビット符号なし整数で浮動小数点数はもちろん固定小数点数も標準では扱えません。ブール関数は数字を返します。  
